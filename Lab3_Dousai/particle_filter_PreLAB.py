...
class ParticleFilter(object):
    ...
    def predict(self, odom):
        ...
        if odom[0]==0 and odom[1]==0 and odom[2]==0:
            ...
            # .... np.random.randn(...)
            #
		x= odom[0]*np.cos(self.p_ang) - odom[1]*np.sin(self.p_ang)
		y= odom[0]*np.sin(self.p_ang) + odom[1]*np.cos(self.p_ang)
	 
            # Increment particle positions in correct frame
            	self.p_xy [0,:]+= x+np.random.randn(self.num)*self.odom_lin_sigma
            	self.p_xy [1,:]+= y+np.random.randn(self.num)*self.odom_lin_sigma
                      

            # Increment angle
            	self.p_ang += odom[2]+self.odom_ang_sigma*np.random.randn(self.num)
