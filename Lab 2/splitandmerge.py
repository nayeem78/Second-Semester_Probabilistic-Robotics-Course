...
#===============================================================================
def splitandmerge(points, split_thres=0.03, inter_thres=0.3, min_points=6, dist_thres=0.12, ang_thres=np.deg2rad(10)):
    '''
...
def split(points, split_thres, inter_thres, min_points, first_pt, last_pt):
    ...
    #
	distance = abs(a * points[0,:] + b * points[1,:] + c) / math.sqrt(math.pow(a,2) + math.pow(b,2))
    ...
    if max_distance > split_thres:
 	...
 	prev = split(points[:,first_pt:max_ind], split_thres, inter_thres, min_points, 0, points[:,first_pt:max_ind].shape[1]-1)
        post = split(points[:,(max_ind+1) : last_pt], split_thres, inter_thres, min_points, 1, points[:,max_ind +1: last_pt].shape[1]-1)
    ...
    # Do not need to split furthermore
        ...
        for i in range(first_pt, last_pt-1):
            ...
            inter_distance = math.sqrt(math.pow(points[0,i] - points[0,i+1],2) + math.pow(points[1,i]-points[1,i+1],2))
            #
            # Check interpoint distance threshold
            if inter_distance > inter_thres:
                #Split line
                
		prev = split(points[:,first_pt : i], split_thres, inter_thres, min_points,0, points[:, first_pt:i].shape[1]-1)
                post = split(points[:,(i+1):last_pt], split_thres, inter_thres, min_points,1, points[:, (i+1):last_pt].shape[1]-1)
